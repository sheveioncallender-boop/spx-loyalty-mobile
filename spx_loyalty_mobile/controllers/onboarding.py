...
from ..models.onboarding import email_value, digest, password_value, AccountLinkError
    ...
    def _page(self, title, description, form=Markup(''), status=200, *, verify_script=False):
        ...
        *{box-sizing:border-box}body{margin:0;padding:28px;font-family:Manrope,Arial,sans-serif;background:#f7f8fa;color:#172840}
        ...
        <p style="font-size:11px;margin-top:30px">Good food. Great company. Since 1987.</p></main>%s</body></html>''') % (
            ...
            Markup('<script src="/spx_loyalty_mobile/static/src/account/verify.js?v=19.0.3.0.4" defer></script>') if verify_script else Markup(''))
    ...
    def _link_page(self, kind, token, password=None, confirmation=None):
        try:
            ...
            if request.httprequest.method == 'POST':
                if kind == 'reset':
                    if password != confirmation:
                        form_error = _('The passwords do not match. Enter your new password in both fields.')
                    else:
                        try:
            ...
            if kind == 'signup':
                ...
                form = (Markup('<p role="alert" style="color:#a83238">%s</p>') % escape(form_error)) + form
            return self._page('A little welcome. Just for you.' if kind == 'signup' else 'Choose a new password.',
                ...
                else 'Choose a new password for your Jenny’s account. After saving, use your new password to sign in.',
                form, status=400 if form_error else 200, verify_script=True)
        except AccountLinkError as error:
            if kind == 'reset':
                title = 'Password already updated.' if error.reason == 'used' else 'Request a new password reset.'
                buttons = Markup('<a class="app-button" href="jennys://account/forgot-password">Request a new password reset</a>')
                if error.reason == 'used':
                    buttons = Markup('<a class="app-button" href="jennys://account/password-updated">Open Jenny’s to sign in</a>') + buttons
                return self._page(title, str(error), buttons, status=200 if error.reason == 'used' else 400)
            return self._page('Request a new verification email.', str(error), status=400)
