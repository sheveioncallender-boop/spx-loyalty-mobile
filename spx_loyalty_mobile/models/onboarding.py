...
class Website(models.Model):
    ...
    spx_mobile_signup_enabled = fields.Boolean('Enable verified signup', groups='base.group_system')
    spx_mobile_signup_program_id = fields.Many2one('loyalty.program', string='New member loyalty program', groups='base.group_system')
    spx_mobile_mail_from = fields.Char('Account email sender', default='no-reply@spxcorp.site', groups='base.group_system')
    spx_mobile_account_origin = fields.Char('Account link origin', groups='base.group_system', help='HTTPS origin only, for example https://jennys.spxcorp.site. Verification and recovery pages are served here.')
    spx_mobile_terms = fields.Text('Account and rewards terms', groups='base.group_system')
    spx_mobile_privacy = fields.Text('Privacy policy', groups='base.group_system')
    spx_mobile_policy_version = fields.Char('Policy version', default='1', groups='base.group_system')
...
class Users(models.Model):
    ...
    spx_mobile_verified_email = fields.Char('Verified app email', readonly=True, copy=False, groups='base.group_system')
    spx_mobile_legacy_access = fields.Boolean('Existing member access', readonly=True, copy=False, groups='base.group_system',
...
class Partner(models.Model):
    ...
    spx_mobile_first_name = fields.Char('App first name', groups='base.group_system')
    spx_mobile_last_name = fields.Char('App last name', groups='base.group_system')
    spx_mobile_birthday = fields.Date('Birthday', groups='base.group_system')
    spx_mobile_sms = fields.Boolean('App promotional SMS', groups='base.group_system')
    spx_mobile_verified_at = fields.Datetime('App email verified', readonly=True, groups='base.group_system')
    spx_mobile_policy_version = fields.Char('Accepted policy version', readonly=True, groups='base.group_system')
    spx_mobile_terms_at = fields.Datetime('Terms accepted', readonly=True, groups='base.group_system')
...
class MobileRegistration(models.Model):
    ...
    @api.model
    def _queue_mail(self, website, email, subject, title, text, link):
    ...
    def _send_link(self):
        ...
        if self.kind == 'signup':
            self.mail_id = self._queue_mail(self.website_id, self.email, 'Verify your Jenny’s account',
                'A little welcome. Just for you.', 'Confirm your email to activate your account and digital loyalty card. This link expires in 24 hours.', link)
        else:
            self.mail_id = self._queue_mail(self.website_id, self.email, 'Reset your Jenny’s password',
                'A fresh start.', 'Choose a new password for your Jenny’s account. This link expires in one hour.', link)
    ...
    def _token(self, website, token, kind):
        ...
        record = self.sudo().search([('website_id', '=', website.id), ('token_hash', '=', digest(token)),
                                    ('kind', '=', kind)], limit=1)
        if not record or record.state != 'pending' or record.expires_at <= fields.Datetime.now():
            raise UserError(_('This link is invalid or expired. Request a new email in the app.'))
        return record

    def _complete(self, password=None):
        self.ensure_one()
        ...
        if self.kind == 'reset':
            ...
            # Bind activation to the password chosen by the person signing up.
            # Possession of an unsolicited email link alone must not activate an attacker's password.
            password_value(password)
            if not self.env['res.users']._crypt_context().verify(password, self.password_hash):
                raise UserError(_('Enter the password you chose when creating this account.'))
            if self._users(self.email):
                raise UserError(_('An account already uses this email. Sign in or use Forgot password.'))
            partners = self.env['res.partner'].sudo().with_context(active_test=False).search([('email', '=ilike', escape_psql(self.email))])
        ...
        self.write({'state': 'done', 'verified_at': fields.Datetime.now(), 'token_hash': False,
                    'receipt_hash': False, 'password_hash': False})
        return user
