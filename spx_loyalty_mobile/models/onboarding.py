...
class AccountExistsUserError(UserError):
    """A signup conflict the app can show with sign-in/recovery actions."""


class AccountLinkError(UserError):
    def __init__(self, kind, reason):
        self.kind, self.reason = kind, reason
        if kind == 'reset':
            messages = {
                'used': 'This password reset link has already been used. Sign in with your new password, or request another password reset through Forgot password in the app.',
                'expired': 'This password reset link has expired. Open Forgot password in the app and request a new password reset link.',
                'replaced': 'A newer password reset was requested. Use the latest password reset email, or open Forgot password in the app to request another.',
                'invalid': 'This password reset link is unavailable. Open Forgot password in the app and request a new password reset link.',
            }
            message = messages[reason]
        else:
            message = 'This verification link is invalid or expired. Request a new verification email in the app.'
...
class MobileRegistration(models.Model):
    ...
    def _signup(self, website, values):
        ...
        if self._users(email):
            ...
            raise AccountExistsUserError(_('An account is already associated with this email address. Sign in or use Forgot password.'))
    ...
    def _send_link(self):
        ...
        if self.kind == 'signup':
            ...
            self.mail_id = self._queue_mail(self.website_id, self.email, 'Reset your Jenny’s password',
                'A fresh start.', 'Choose a new password for your Jenny’s account. This link is valid for 24 hours and can be used once. If you request another reset, use the newest email.', link, registration=self)
    ...
    def _token(self, website, token, kind):
        if not isinstance(token, str) or not 30 <= len(token) <= 100:
            raise AccountLinkError(kind, 'invalid')
        ...
        if not record:
            raise AccountLinkError(kind, 'invalid')
        if not completed_signup:
            if record.state == 'done':
                raise AccountLinkError(kind, 'used')
            if record.expires_at <= fields.Datetime.now():
                raise AccountLinkError(kind, 'expired')
            if record.state != 'pending':
                raise AccountLinkError(kind, 'replaced')
        return record
    ...
    def _complete(self, token, password=None):
        ...
        if (not isinstance(token, str) or not 30 <= len(token) <= 100
            ...
            raise AccountLinkError(self.kind, 'invalid')
        if self.kind == 'signup' and self.state == 'done' and self.user_id:
            return self.user_id
        if self.state == 'done':
            raise AccountLinkError(self.kind, 'used')
        if self.expires_at <= fields.Datetime.now():
            raise AccountLinkError(self.kind, 'expired')
        if self.state != 'pending':
            raise AccountLinkError(self.kind, 'replaced')
        ...
        self.write({'state': 'done', 'verified_at': fields.Datetime.now(),
                    # Retain only the digest so a repeated POST can explain that
                    # the reset succeeded. A done reset can never change a password.
    ...
    @api.autovacuum
    def _gc(self):
        self.sudo().search([('kind', '=', 'reset'), ('state', '!=', 'pending'),
                           ('expires_at', '<', fields.Datetime.now())]).write({'token_hash': False})
