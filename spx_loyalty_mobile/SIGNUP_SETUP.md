...
Android **0.5.1+10** · Odoo connector **19.0.3.0.3** · 13 September 2026
...
3. Upgrade **Jenny’s Rewards** (`spx_loyalty_mobile`) in Odoo, or use the existing Cloudpepper module auto-upgrade process. Confirm installed version **19.0.3.0.3**. Restart workers if the deployment process has not reloaded them. This release adds optional registration handoff fields, so deploying code without the module upgrade is insufficient.
4. Install **Jennys-Android-0.5.1-arm64.apk** over the current app. It uses the existing test certificate; an uninstall should not be necessary. This is an ARM64 Android test build, minimum Android 7.0.
...
8. Test signup with the same email using different capitalization. There must be no additional user. The existing-account email directs the customer to sign in or use recovery.
...
6. Sign out and check that the old password fails. A used reset link must fail on reuse, and requesting another reset invalidates the earlier pending reset link. Resetting a password also invalidates outstanding signup sign-in handoffs for that user.

Verification links expire after 24 hours; recovery links after one hour. Resend has a one-minute cooldown and replaces the previous verification token. Use the newest email. Repeated account requests are limited; allow an hour if testing reaches the limit.
...
Completed locally: **28 focused Flutter tests**, static analysis, **23 Python controller/service/ORM checks**, and JavaScript form-flow checks. The Python checks simulate database records; the JavaScript checks simulate browser events. They cover origin handling, native CSRF dispatch, missing/expired links, receipt binding, wrong-device/wrong-user rejection, single-use grants, signup data, resume sign-in, reset return, session storage and existing order/card behavior.
