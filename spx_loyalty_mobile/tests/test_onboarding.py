...
class TestMobileOnboarding(TransactionCase):
    ...
    def test_verify_creates_portal_and_card_once(self):
        ...
        user = record._complete(token=self.token(record))
        ...
        self.assertEqual(self.service._token(self.website, token, 'signup'), record)
        self.assertEqual(record._complete(token=token), user)
    ...
    def test_existing_contact_card_and_points_preserved(self):
        ...
        record, _ = self.signup()
        record._complete(token=self.token(record))
    ...
    def test_second_signup_cannot_overwrite_existing_user_password(self):
        ...
        with self.assertRaisesRegex(UserError, 'already associated'):
    ...
    def test_reset_changes_native_password_and_is_single_use(self):
        ...
        token = self.token(reset)
        # Still usable after the old one-hour limit has elapsed.
        self.assertGreater(reset.expires_at, fields.Datetime.now()+timedelta(hours=23))
        user = record.user_id.with_user(record.user_id)
        reset._complete(token=token, password='Fresh-password-234!')
        self.assertEqual(user._check_credentials({'type': 'password', 'password': 'Fresh-password-234!'}, {'interactive': True})['uid'], user.id)
        with self.assertRaises(AccessDenied):
            user._check_credentials({'type': 'password', 'password': self.values['password']}, {'interactive': True})
        self.env.cr.execute('SELECT password FROM res_users WHERE id=%s', (record.user_id.id,))
        stored = self.env.cr.fetchone()[0]
        ...
        with self.assertRaises(UserError):
            reset._complete(token=self.token(reset), password='Another-password-234!')
    ...
    def test_verification_uses_email_token_and_preserves_signup_password(self):
        record, _ = self.signup()
        with self.assertRaises(UserError):
            record._complete(token='wrong-token-'+'x'*32)
        self.assertFalse(self.service._users(record.email))
        stored_hash = record.password_hash
        user = record._complete(token=self.token(record))
        self.env.cr.execute('SELECT password FROM res_users WHERE id=%s', (user.id,))
        self.assertEqual(self.env.cr.fetchone()[0], stored_hash)
        self.assertEqual(record.state, 'done')

    def test_link_rotation_between_lookup_and_completion_is_rejected(self):
        record, receipt = self.signup()
        original = self.token(record)
        self.service._token(self.website, original, 'signup')
        record.sent_at = fields.Datetime.now()-timedelta(minutes=2)
        self.service._resend(self.website, receipt)
        with self.assertRaises(UserError):
            record._complete(token=original)
        self.assertFalse(self.service._users(record.email))
        record._complete(token=self.token(record))
        self.assertEqual(record.state, 'done')

    def test_email_uniqueness_includes_other_login_and_case(self):
        existing = self.env['res.users'].with_context(no_reset_password=True).create({
            'name': 'Existing member', 'login': 'different-login', 'email': 'New.Member@Example.Com',
            'group_ids': [Command.set([self.env.ref('base.group_portal').id])],
        })
        self.assertEqual(self.service._users(self.values['email']), existing)
        with self.assertRaisesRegex(UserError, 'already associated'):
            self.service._signup(self.website, self.values)
        self.assertFalse(self.service.search([('email', '=', self.values['email'])]))
        self.assertEqual(self.service._users(self.values['email']), existing)

    def test_handoff_needs_original_receipt_and_is_single_use(self):
        record, receipt = self.signup()
        user = record._complete(token=self.token(record))
        code = record._issue_handoff()
        with self.assertRaises(AccessDenied):
            self.service._handoff_registration(self.website.id, code, 'wrong-'+'x'*40, consume_for=user.id)
        self.assertEqual(self.service._handoff_registration(self.website.id, code, receipt, consume_for=user.id), record)
        with self.assertRaises(AccessDenied):
            self.service._handoff_registration(self.website.id, code, receipt, consume_for=user.id)
